...
size_t estimate_gzip_uncompressed_size(const uint8_t* compressed, size_t comp_size)
{
	if (comp_size < 4) return 0;  // 文件过小

	// 读取最后4字节 (小端序)
	uint32_t isize = *((uint32_t*)(compressed + comp_size - 4));
#if MINIZ_LITTLE_ENDIAN
	return isize;  // 小端系统直接使用
#else
	return ((isize >> 24) & 0xFF) |   // 字节序转换
		((isize >> 8) & 0xFF00) |
		((isize << 8) & 0xFF0000) |
		((isize << 24) & 0xFF000000);
#endif
