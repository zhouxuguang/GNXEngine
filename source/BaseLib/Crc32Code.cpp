...
uint32_t Calculate_CRC32(const void *pStart, uint32_t uSize)
{
...
//           iLenIn: 数据长度              
// 输出参数: pCRCOut: 2字节校验值    
 uint16_t Calculate_CRC16(const void* pDataIn, uint32_t iLenIn)   
{   
	 uint16_t wResult = 0;   
	 uint16_t wTableNo = 0;

	const uint8_t* pBytes = (const uint8_t*)pDataIn;

	for(int i = 0; i < iLenIn; i++)   
	{   
		wTableNo = ((wResult & 0xff) ^ (pBytes[i] & 0xff));   
