...
cbuffer AtmosphereViewCB : register(b1)
{
    ...
    // 参考 Demo 中用于演示光柱的“额外几何体”（球体 + 地面），均为大气单位
    float4   sphere_center_radius;   // xyz = 球心, w = 球半径
    float4   sphere_albedo_pad;      // xyz = 球体反照率
    float4   ground_albedo_pad;      // xyz = 地面着色反照率
};
...
// ---- 参考 Demo 的“额外几何体”（球体/地面）辅助函数 ----
// 球心、半径、反照率等场景参数全部由 AtmosphereViewCB 传入，着色器内不再写死。
float GetSunVisibility(float3 pos, float3 sun_direction,
                       float3 sphereCenter, float sphereRadius, float sunSizeX)
{
    float3 p = pos - sphereCenter;
    float p_dot_v = dot(p, sun_direction);
    float p_dot_p = dot(p, p);
    float ray_sphere_center_squared_distance = p_dot_p - p_dot_v * p_dot_v;
    float distance_to_intersection = -p_dot_v - sqrt(sphereRadius * sphereRadius - ray_sphere_center_squared_distance);
    if (distance_to_intersection > 0.0)
    {
        float ray_sphere_distance = sphereRadius - sqrt(ray_sphere_center_squared_distance);
        float ray_sphere_angular_distance = -ray_sphere_distance / p_dot_v;
        return smoothstep(1.0, 0.0, ray_sphere_angular_distance / sunSizeX);
    }
    return 1.0;
}

float GetSkyVisibility(float3 pos, float3 sphereCenter, float sphereRadius)
{
    float3 p = pos - sphereCenter;
    float p_dot_p = dot(p, p);
...
{
    float3 pos = camera - sphereCenter;
    float pos_dot_sun = dot(pos, sun_direction);
    float view_dot_sun = dot(view_direction, sun_direction);
    float k = sunSizeX;
    float l = 1.0 + k * k;
    float a = 1.0 - l * view_dot_sun * view_dot_sun;
    float b = dot(pos, view_direction) - l * pos_dot_sun * view_dot_sun - k * sphereRadius * view_dot_sun;
    float c = dot(pos, pos) - l * pos_dot_sun * pos_dot_sun -
              2.0 * k * sphereRadius * pos_dot_sun - sphereRadius * sphereRadius;
    ...
    if (discriminant > 0.0)
    {
        ...
        float d_apex = -(pos_dot_sun + sphereRadius / k) / view_dot_sun;
        if (view_dot_sun > 0.0)
        {
...
float4 PS(VS_OUTPUT input) : SV_Target0
{
    ...
    // 场景“额外几何体”（球体 + 地面）参数，由 C++ 侧（AtmosphereComponent）传入
    float3 sphere_center = sphere_center_radius.xyz;
    float  sphere_radius = sphere_center_radius.w;
    float3 sphere_albedo = sphere_albedo_pad.xyz;
    float3 ground_albedo = ground_albedo_pad.xyz;
    ...
    float shadow_in;
    float shadow_out;
    GetSphereShadowInOut(view_direction, sun_direction, camera,
                         sphere_center, sphere_radius, sun_size.x, shadow_in, shadow_out);

    // 太阳接近地平线时淡出光柱
    float lightshaft_fadein_hack = smoothstep(0.02, 0.04, dot(normalize(camera - earth_center), sun_direction));

    // ---- 球体 ----
    float3 p = camera - sphere_center;
    float p_dot_v = dot(p, view_direction);
    float p_dot_p = dot(p, p);
    float ray_sphere_center_squared_distance = p_dot_p - p_dot_v * p_dot_v;
    float discriminant = sphere_radius * sphere_radius - ray_sphere_center_squared_distance;
    ...
    if (discriminant >= 0.0)
    {
        ...
        if (distance_to_intersection > 0.0)
        {
            float ray_sphere_distance = sphere_radius - sqrt(ray_sphere_center_squared_distance);
            float ray_sphere_angular_distance = -ray_sphere_distance / p_dot_v;
            ...
            float3 normal = normalize(pos - sphere_center);
            float3 sky_irradiance;
            float3 sun_irradiance = GetSunAndSkyIrradiance(pos - earth_center, normal, sun_direction, sky_irradiance);
            sphere_radiance = sphere_albedo * (1.0 / PI) * (sun_irradiance + sky_irradiance);
    ...
    if (discriminant >= 0.0)
    {
        ...
        if (distance_to_intersection > 0.0)
        {
            ...
            ground_radiance = ground_albedo * (1.0 / PI) * (
                sun_irradiance * GetSunVisibility(pos, sun_direction, sphere_center, sphere_radius, sun_size.x) +
                sky_irradiance * GetSkyVisibility(pos, sphere_center, sphere_radius));
