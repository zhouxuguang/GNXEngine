...
EditorSettings::EditorSettings()
    : mSettings(std::make_unique<QSettings>(QSettings::IniFormat, QSettings::UserScope, "GNXEngine",
                                            "GNXEditor"))
{
}
...
void EditorSettings::Load()
{
    ...
    if (mSettings->value("settings/version", currentVersion).toInt() > currentVersion)
        return;
    mRecentProjects = mSettings->value("projects/recent").toStringList();
    QStringList validProjects;
    for (const QString &project : std::as_const(mRecentProjects))
    {
        const QFileInfo info(project);
        if (info.isFile() && info.suffix().compare("gnxproj", Qt::CaseInsensitive) == 0 &&
            !validProjects.contains(info.absoluteFilePath(), Qt::CaseInsensitive))
            validProjects.push_back(info.absoluteFilePath());
    }
    mRecentProjects = validProjects;
    mLastProject = mSettings->value("projects/last").toString();
    if (!QFileInfo(mLastProject).isFile())
        mLastProject.clear();
