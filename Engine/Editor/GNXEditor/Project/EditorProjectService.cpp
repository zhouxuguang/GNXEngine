...
bool EditorProjectService::CreateProject(const ProjectCreateRequest &request) {
  const QString root =
      QDir(request.parentDirectory).filePath(request.projectName);
  if (!GNXEngine::ProjectManager::GetInstance().CreateNewProject(
          root.toStdString(), request.projectName.toStdString())) {
...
bool EditorProjectService::OpenProject(const QString &file) {
  if (!GNXEngine::ProjectManager::GetInstance().OpenProject(
          file.toStdString())) {
...
bool EditorProjectService::SaveProject() {
  if (!HasOpenProject() ||
      !GNXEngine::ProjectManager::GetInstance().SaveProject()) {
