...
public:
    ...
    static void FreeBitmapData(void* pData)
    {
        if (pData)
        {
            free(pData);
    ...
    bool LoadFromMemory(const void* pBuffer, unsigned int nSize)
    {
        ...
        unsigned char* pTempBuffer = (unsigned char*)pBuffer;
        unsigned char* pTempBufferBegin = (unsigned char*)pBuffer;

        memcpy(&m_BitmapFileHeader, pTempBuffer, BITMAP_FILEHEADER_SIZE);
        if (m_BitmapFileHeader.Signature != BITMAP_SIGNATURE)
        {
            return false;
        }

        pTempBuffer += BITMAP_FILEHEADER_SIZE;

        memcpy(&m_BitmapHeader, pTempBuffer, sizeof(BITMAP_HEADER));

        /* Load Color Table */

        pTempBuffer += m_BitmapHeader.HeaderSize;
        ...
        memcpy(ColorTable, pTempBuffer, sizeof(BGRA) * m_BitmapHeader.ClrUsed);

        /* ... Color Table for 16 bits images are not supported yet */

        m_BitmapSize = GetWidth() * GetHeight();
        //m_BitmapData = new RGBA[m_BitmapSize];
        m_BitmapData = (RGBA*)malloc(m_BitmapSize * sizeof(RGBA));
        memset(m_BitmapData, 0, m_BitmapSize * sizeof(RGBA));

        unsigned int LineWidth = ((GetWidth() * GetBitCount() / 8) + 3) & ~3;
        uint8_t *Line = new uint8_t[LineWidth];

        //定位数据偏移
        pTempBuffer = pTempBufferBegin + m_BitmapFileHeader.BitsOffset;
        ...
        if (m_BitmapHeader.Compression == 0) {
            for (unsigned int i = 0; i < GetHeight(); i++) {
                ...
                memcpy(Line, pTempBuffer, LineWidth);
                pTempBuffer += LineWidth;
            ...
            //当前数据的偏移量
            unsigned int nOffset = m_BitmapFileHeader.BitsOffset;

            while (nOffset != nSize) {
                memcpy(&Count, pTempBuffer, sizeof(uint8_t));
                pTempBuffer += sizeof(uint8_t);

                memcpy(&ColorIndex, pTempBuffer, sizeof(uint8_t));
                pTempBuffer += sizeof(uint8_t);
                ...
                else if (Count == 0)
                {
                    ...
                    else if (Flag == 2)
                    {
                        ...
                        memcpy(&rx, pTempBuffer, sizeof(char));
                        pTempBuffer += sizeof(char);
                        //file.read((char*) &ry, sizeof(char));
                        memcpy(&ry, pTempBuffer, sizeof(char));
                        pTempBuffer += sizeof(char);
                        x += rx;
                        y += ry;
                    }
