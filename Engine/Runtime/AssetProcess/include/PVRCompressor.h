...
void ASSET_PROCESS_API CompressPVRRGB4Bpp(uint8_t* result, const uint8_t* input, uint32_t width, uint32_t height);

// PVR数据压缩,input是RGBA4个通道的数据
void ASSET_PROCESS_API CompressPVRRGBA4Bpp(uint8_t* result, const uint8_t* input, uint32_t width, uint32_t height);
