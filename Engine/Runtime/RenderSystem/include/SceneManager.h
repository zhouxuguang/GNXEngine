...
public:
    ...
    /**
     * 解绑并销毁当前相机控制器，之后引擎不再自动接管相机 / 转发输入事件。
     *
     * 适用于「自行管理相机」的 demo：CreateCamera() 会默认挂上一个轨道相机控制器，
     * 它会用世界 Y-up 重新推导相机的 yaw/pitch 并重建视图矩阵；而像大气散射 demo
     * 这类使用 Z-up 基向量的相机，一旦被它接管，画面就会整体滚转约 90°。
     * 这类 demo 在创建相机后调用本接口即可完全接管相机。
     */
