...
static RCTexture2DPtr TextureFromFile(const char *filename, bool isSRGB)
{
    ...
    // Vulkan/Metal 对 3 通道 R8G8B8 格式支持有限（optimal tiling 时多数 GPU 不支持）
    // 统一将 RGB8/SRGB8 转换为 RGBA8/SRGB8_ALPHA8，alpha 通道填充 255
    ...
    if (srcFormat == imagecodec::FORMAT_RGB8 || srcFormat == imagecodec::FORMAT_SRGB8)
    {
        ...
        for (uint32_t y = 0; y < height; ++y)
        {
            ...
            for (uint32_t x = 0; x < width; ++x)
            {
                ...
                dstRow[x * 4 + 3] = 255;   // alpha = 255（完全不透明）
            }
        }
    ...
    if (isSRGB)
    {
        // 非 RGB8 的 sRGB 格式标签转换（RGBA8 → SRGB8_ALPHA8）
        auto fmt = image->GetFormat();
        if (fmt == imagecodec::FORMAT_RGBA8)
        {
...
static RCTexture2DPtr TextureFromMemory(const uint8_t* pImageData, uint32_t dataSize)
{
    ...
    // Vulkan/Metal 对 3 通道 R8G8B8 格式支持有限，统一转换为 RGBA
    ImagePixelFormat srcFormat = image->GetFormat();
    if (srcFormat == imagecodec::FORMAT_RGB8 || srcFormat == imagecodec::FORMAT_SRGB8)
    {
        uint32_t width = image->GetWidth();
...
{
    {
        ...
        channels[kShaderChannelWeight].stride = sizeof(Vector4f);
    }
