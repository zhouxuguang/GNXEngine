...
#undef CASE
		...
		{
#if __cplusplus >= 202002L && !defined(__APPLE__) && !defined(__ANDROID__)
		...
		[[nodiscard]] auto stringify(const ResourceNode& node, const ResourceEntry& entry)
		{
			std::ostringstream oss;
			oss << "{ {<B>" << node.getName() << "</B>";
	...
	void Writer::operator()(const PassNode& node, const std::vector<ResourceNode>& resourceNodes) 
	{
		...
		auto& edge = graph.edges.emplace_back(Graph::Edge{ std::move(key), colors.edge.write });
	...
	void Writer::operator()(const ResourceNode& node, const ResourceEntry& entry, const std::vector<PassNode>& passNodes) 
	{
		auto key = makeKey(node);
		...
		auto& edge = graph.edges.emplace_back(Graph::Edge{ std::move(key), colors.edge.read });
