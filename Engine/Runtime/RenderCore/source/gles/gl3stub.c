...
static void open_libgl(void)
{
    // we are always linked to OpenGLES.framework statically, so it is already loaded and could be found by id
    bundle = CFBundleGetBundleWithIdentifier(CFSTR("com.apple.opengles"));
    assert(bundle != NULL);
...
#define FIND_PROC(s) s = (void*)get_proc(#s);
